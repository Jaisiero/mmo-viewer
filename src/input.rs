...
//!   B                — toggle the shard-boundary overlay
//!   Esc              — quit
//!
...
pub struct InputState {
    ...
    /// Whether the shard-boundary overlay is on. Toggled by the `B`
    /// key. Defaults to `true` because the first thing a developer
    /// debugging a multi-shard layout wants to see is the seam lines.
...
pub fn poll(state: &mut InputState) -> Vec<GuiCmd> {
    ...
    // Boundary overlay toggle. Edge-triggered so a held `B` doesn't
    // strobe the overlay on/off every frame.
    if is_key_pressed(KeyCode::B) {
        state.show_boundaries = !state.show_boundaries;
    }
